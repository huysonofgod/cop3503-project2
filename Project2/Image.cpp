...
void Image::LoadImageFile(string fileName)
{
	...
	if (file.is_open())
	{
		...
		// Read Image Pixels count many times and push to the vector
...
void Image::WriteImage(string outFileName)
{
	...
	if (file.is_open())
	{
		...
		uchar zero = (uchar) '0';
		// Write Image Pixels from the vector
...
/*		Helper Functions		*/
uint Image::ConvertCharToInt(uchar _char)
{
	return (uint)(_char - '\0');
}

uchar Image::ConvertInttoChar(uint _int)
{
	return (uchar)(_int + '\0');
}
